...
(function($, window, document, undefined) {
  ...
  Waterfall.prototype = {
      ...
      var options = this.options,
        ...
        containerWidth =
          $container[0].tagName === "BODY"
            ? $container.width() - 20
            : $container.width(), // if $container[0].tagName === 'BODY', fix browser scrollbar
    ...
    layout: function($content, callback) {
      var options = this.options,
        $items = this.options.isFadeIn
          ? this._getItems($content)
              .css({ opacity: 0 })
              .animate({ opacity: 1 })
          : this._getItems($content),
        styleFn =
          this.options.isAnimated && this.options.state.isResizing
            ? "animate"
            : "css",
      ...
      if (align === "center") {
        fixMarginLeft =
          (this.$element.width() - colWidth * len - gutterWidth * (len - 1)) /
          2;
    ...
    _requestData: function(callback) {
      ...
      if (maxPage !== undefined && curPage > maxPage) {
        ...
        options.callbacks.loadingFinished(
          this.$loading,
    ...
    _handleResponse: function(data, callback) {
      ...
      if (!checkImagesLoaded) {
        ...
        self.options.callbacks.loadingFinished(
          self.$loading,
    ...
    _nearbottom: function() {
      var options = this.options,
        ...
        distanceFromWindowBottomToMinColBottom =
          $window.scrollTop() +
          $window.height() -
