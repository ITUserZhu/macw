...
$(() => {
  ...
  $submit.click(event => {
    ...
    if (formData.url === "") {
      ...
      !urlReg.test(formData.url) ||
      formData.url.indexOf("__host.com") < 0
    ) {
    ...
    if (
      formData.contact_way_e === "" &&
      formData.contact_way_q === "" &&
      formData.contact_way_p === ""
    ) {
      ...
      if (
        formData.contact_way_e !== "" &&
        !emailReg.test(formData.contact_way_e)
      ) {
        $errorTips.eq(2).text("请填写正确邮箱格式");
      }
      if (
        formData.contact_way_q !== "" &&
        !qqReg.test(formData.contact_way_q)
      ) {
        $errorTips.eq(2).text("请填写正确qq号码格式");
      }
      if (
        formData.contact_way_p !== "" &&
        !phoneReg.test(formData.contact_way_p)
      ) {
