  ...
  class PersonalOperate {
    ...
    initColNavEvent() {
      this.collectHook.nav.on("click", "span", e => {
        ...
        switch (
          this.collectHook.contBoxs.eq(index)[0].className.split(" ")[0]
        ) {
          case "soft":
            break;
          case "plugin":
    ...
    dealHtml(data, perPage, curPage, pagiHook, wrap, temp, type) {
      ...
      total >= perPage &&
        (curPage % perPage == 0 || pagiTotal != curMsgLength) &&
    ...
    initDownNavEvent() {
      this.downloadHook.nav.on("click", "span", e => {
        ...
        switch (
          this.downloadHook.contBoxs.eq(index)[0].className.split(" ")[0]
        ) {
          case "soft":
            break;
          case "plugin":
    ...
    getDownData(model_id = this.modelIds.mac, page_size = 10, page = 1) {
      ...
      switch (model_id) {
        ...
        case this.modelIds.plugin:
          ...
          tempHtml = _this.pluginHtml.replace(
        ...
        case this.modelIds.template:
          ...
          tempHtml = _this.templateHtml.replace(
            '<div class="del">删除</div>',
            ""
          );
          break;
        case this.modelIds.material:
          ...
          tempHtml = _this.materialHtml.replace(
            '<div class="del">删除</div>',
            ""
          );
          isMaterial = true;
    ...
    initHistoryNavEvent() {
      this.historyHook.nav.on("click", "span", e => {
        ...
        switch (
          this.historyHook.contBoxs.eq(index)[0].className.split(" ")[0]
        ) {
          case "soft":
            break;
          case "plugin":
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (data && typeof data == "object") {
              // 展示数据
              _this.newsHook.table
                .empty()
                .append(_this.showReportedList(data.info));
              // 生成翻页
              let totalPage = Math.ceil(data.total / 10);
              totalPage > 1 &&
                _this.createPagi(totalPage, page, _this.newsHook.pagi) &&
                _this.newsHook.pagi.attr("data-total", totalPage);
            } else {
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (data && typeof data == "object" && data.data) {
              ...
              let totalPage = Math.ceil(data.total / 10);
              totalPage > 1 &&
                _this.createPagi(totalPage, page, _this.sysHooks.pagi) &&
                _this.sysHooks.pagi.attr("data-total", totalPage);
            } else {
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (data.data.length && typeof data == "object") {
              // 展示数据
              _this.rechargeHook.table
                .empty()
                .append(_this.showRechargeList(data.data));
              // 生成翻页
              let totalPage = Math.ceil(data.total / 10);
              totalPage > 0 &&
                _this.createPagi(totalPage, page, _this.rechargeHook.pagi) &&
                _this.rechargeHook.pagi.attr("data-total", totalPage);
            } else {
    ...
    showRechargeList(data) {
      ...
      $.each(data, function(i, ele) {
        html += _this.rechargeHtml
          ...
          .replace(
            "{ time }",
            UTIL.formatDate(ele["add_time"] * 1000, "Y-m-d")
          );
      });
    ...
    initRechargeEvent() {
      this.rechargeHook.form.on("submit change", e => {
        e.preventDefault();
        this.getRechargeData();
      });
    }
      ...
      $.ajax({
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (data && typeof data == "object") {
              // 展示数据
              _this.consumeHook.table
                .empty()
                .append(_this.showConsumeList(data.info));
              // 生成翻页
              let totalPage = Math.ceil(data.total / 10);
              totalPage > 0 &&
                _this.createPagi(totalPage, page, _this.consumeHook.pagi) &&
                _this.consumeHook.pagi.attr("data-total", totalPage);
            } else {
    ...
    showConsumeList(data) {
      ...
      $.each(data, function(i, ele) {
        html += _this.consumeHtml
          ...
          .replace(
            "{{ content }}",
            ele.url ? '<a href="' + ele.url + '">' + ele.msg + "</a>" : ele.msg
          )
    ...
    // 消费记录事件注册
    initConsumeEvent() {
      this.consumeHook.form.on("submit change", e => {
        e.preventDefault();
        this.getConsumeData();
      });
    }
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (
              ...
              typeof data.coupon_info == "object"
            ) {
              // 展示数据
              _this.couponHook.wrap
                .empty()
                .append(_this.showCouponList(data.coupon_info));
              // 生成翻页
              let totalPage = Math.ceil(data.total / 15);
              totalPage > 0 &&
                _this.createPagi(totalPage, page, _this.couponHook.pagi) &&
                _this.couponHook.pagi.attr("data-total", totalPage);
            } else {
        ...
        success: function(msg) {
          if (msg.code == 200) {
            ...
            if (data.data.length && typeof data.data == "object") {
              // 展示数据
              _this.shareHook.table
                .empty()
                .append(_this.showShareList(data.data));
              // 生成翻页
              let totalPage = Math.ceil(data.num / 10);
              totalPage > 1 &&
                _this.createPagi(totalPage, page, _this.shareHook.pagi) &&
                _this.shareHook.pagi.attr("data-total", totalPage);
            } else {
