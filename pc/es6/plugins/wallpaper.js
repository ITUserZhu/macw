/*
 * @Author: Liliang Zhu 
 * @Date: 2019-11-21 11:31:33 
 * @Last Modified by:   Liliang Zhu 
 * @Last Modified time: 2019-11-21 11:31:33 
 * 壁纸自动播放功能
 */

$.fn.extend({
  hoverCheck(speed = 1000, time = 500) {
    $.packTimer = setTimeout(() => {
      ...
      if (imgLen > 1) {
        let curIndex = 0,
          curZindex = 0;
        $imgs.first().addClass('active').siblings().removeClass('active');
        // 循环判断加载更多图片
        $imgs.each((i, v) => {
          if (!$(v).attr('src')) {
        ...
        $.packIntervalTimer = setInterval(() => {
          ...
          // 通过类名控制展示隐藏
          $imgs.eq(curIndex).addClass('active').siblings().removeClass('active');
          // 层级控制柔和切换效果
