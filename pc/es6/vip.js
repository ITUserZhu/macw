...
// api接口
import { VIP_APIS } from "./api";
// 引入工具
import { toggleActive, alertBox, confirmBox } from "./util";
...
$(function() {
  class VipRecharge {
    ...
    initDom() {
      ...
      const $defaultLi = this.vipHooks.$vipWrap
        .find(".combo")
        .children("li")
        .eq(this.defaultNum);
    ...
    initEvent() {
      ...
      this.vipHooks.$vipWrap.on(
        "click",
        ".item",
        this.checkRecharge.bind(this)
      );
    ...
    showType(e) {
      ...
      toggleActive([
        this.vipHooks.$vipWrap.children("ul").eq(type),
        $(e.target)
      ]);
      this.defaultData.id = this.vipHooks.$vipWrap
        .children("ul")
        .eq(type)
        .find("li:first-child")
        .data("id");
    ...
    callPay() {
      ...
      const { id, coupon_relation_id } = this.defaultData;
    ...
    payAjax(data) {
      for (let index = 0; index < 2; index++) {
        let dom = index
          ? this.vipHooks.$wxPay.find("img")
          : this.vipHooks.$aliPay.find("img");
